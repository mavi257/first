...
$r=mysqli_fetch_array($re,MYSQLI_ASSOC);
  $s56 = "SELECT * FROM category where cat_id='$cat_id' ";
...
<li id="wp-admin-bar-my-account" class="menupop with-avatar no-grav"><a class="ab-item" aria-haspopup="true" href=""><img alt="" src="upload/pr.png" class="avatar avatar-32" height="32" width="32"><span class="ab-text">Me</span></a>			<div class="ab-sub-wrapper">		<ul id="wp-admin-bar-user-actions" class="ab-submenu">
    ...
    <div class="col-md-8">
       <?php
$sql1 = "SELECT * FROM post  where cat_id=$cat_id order by postid DESC   LIMIT $start_from, $per_page";
    $result2 = $conn->query($sql1);
if ($result2->num_rows > 0){?>
     <h3 class="page-header">
                   <a href='index'>Home</a> / <a href=''><?php echo $r56['category']?></a>
                </h3>
...
$query345 = "SELECT * FROM post where cat_id='$cat_id'";
$result345 = mysqli_query($conn, $query345);
if ($result345->num_rows > 0 && $result345->num_rows > 5){
    $total_records = mysqli_num_rows($result345);
...
echo "         <li class='next'><a href='catpage?cat_id=$cat_id&page=".$i."'>".'Older &rarr;'."</a> </li></ul> ";
}
...
echo "     <ul class='pager'>    <li class='previous'><a href='catpage?cat_id=$cat_id&page=".$i2."'>".'&larr; Newer'."</a> </li> ";
...
echo "         <li class='next'><a href='catpage?cat_id=$cat_id&page=".$i1."'>".'Older &rarr;'."</a> </li></ul> ";
